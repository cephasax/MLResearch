	...
	public static void main(String[] args) throws IOException {
		...
		for (String alg : algs) {
			for (int thread = 0; thread < numThreads; thread++) {
				PrintStream out = new PrintStream("V2_Exp" + alg + thread + ".sh");
				out.println(
				...
				for (String base : bases[thread]) {
					for (int seed = 0; seed < 3; seed++) {
						out.println(String.format("java $args -cp \".:$jars\" $app \"%s\" %s %s %s", base, alg, seeds[seed], "V2_Res" + alg + thread + ".txt"));
					}
				}
				for (String base : bases[thread]) {
					for (int seed = 3; seed < seeds.length; seed++) {
						out.println(String.format("java $args -cp \".:$jars\" $app \"%s\" %s %s %s", base, alg, seeds[seed], "V2_Res" + alg + thread + ".txt"));
					}
				}
